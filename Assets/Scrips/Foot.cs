...
public class Foot : MonoBehaviour
{
    ...
    Vector3 _debug_halfway_point = Vector3.zero; // Used to store the position for the halfway target, for drawing debug gizmos
    ...
    void Update()
    {
        if (_state == StepState.planted && _opposingFoot.isPlanted)
        {
            ...
            if (Vector3.Distance(transform.position, _footPlacementTarget.position) > _stepSize) //If this foot is at its plant position
            {
                ...
                _debug_halfway_point =_nextTarget; // Assign for debugging. TODO: remove before release
        ...
        if(_state == StepState.lifting)
        {
            ...
            if (Vector3.Distance(transform.position, _nextTarget) < _postitionTolerance) 
            {
                ...
                _nextTarget = _footPlacementTarget.position;
 ...
 //position , then divide by 2 and add to the current position to get a point halfway to your destination.
        Vector3 halfwayPoint = transform.position +( _footPlacementTarget.position - transform.position ) /2;
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawSphere(_debug_halfway_point, .1f);
